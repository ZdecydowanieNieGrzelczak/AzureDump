...
def init_memory(nr_of_runs):
    ...
    while i < nr_of_runs:
        ...
        next_state, reward, is_done, should_oracle = env.step(action)
        memory.append((state, action, 0.25, reward, next_state, is_done, should_oracle))
        state = next_state
        if is_done:
            i += 1
            state = env.reset()
...
def run_episode():
    ...
    for i in range(master_params["MAX_STEPS"]):
        ...
        if memory.is_multiprocessing:
            obs.append((state, action, prob, reward, next_state, is_done, should_oracle))
        else:
            memory.append((state, action, prob, reward, next_state, is_done, should_oracle))
...
def learn():
    if memory.is_prioritized:
        sampled_batch = memory.sample_batch(master_params["BATCH_SIZE"], memory_dict["current_beta"])
        batch, ISWeights = sampled_batch
    else:
        batch = memory.sample_batch(master_params["BATCH_SIZE"])
        ISWeights = [1 for i in range(master_params["BATCH_SIZE"])]
    ...
    for i, sample in enumerate(batch):
        # print(sample)
        state, action, old_prob, reward, next_state, is_done, should_oracle = sample
